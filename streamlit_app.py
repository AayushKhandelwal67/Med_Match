...
if st.session_state.recommendation_made:
    # Context Expander
    with st.expander("🔍 Show Analysis Details (Context & Codes)"):
        rag_context_display = st.session_state.get('rag_context_to_show', "N/A")
        icd_codes_display = st.session_state.get('icd_codes_to_show', "N/A")
        st.markdown("**Symptoms Used:**"); st.text(st.session_state.get('symptom_history', "").strip())
        st.markdown("**Matched ICD10 Codes:**"); st.text(icd_codes_display)
        st.markdown("**Retrieved Context:**")
        if isinstance(rag_context_display, str) and rag_context_display.strip() and not rag_context_display.startswith("N/A") and not rag_context_display.startswith("Error"):
            chunks = rag_context_display.split("\n---\n")
            for i, chunk_text in enumerate(chunks[:RAG_K]):
                 st.markdown(f"--- Chunk {i+1} ---")
                 st.text_area(f"Chunk_{i+1}", value=chunk_text.strip(), height=150, disabled=True, key=f"rag_chunk_{i}")
        else: st.text(rag_context_display)
